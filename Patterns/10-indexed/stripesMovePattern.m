...
%GENERAL PARAMETERS
    ...
    pattern.y_size      = 8;
%MAKE 'pattern.Pats'
%     pattern.Pats    =   zeros(pattern.x_panels*pattern.x_size,pattern.y_panels*...
%         pattern.y_size,pattern.x_num,pattern.y_num);
    pattern.Pats = zeros(2, 96, pattern.x_num,pattern.y_num);
%--------------------------------------------------------------------------
%SPECIFIC PARAMETERS
    pattern.name = 'nobelStripesMove';
...
patternTemp(:, :, 1) = pattern_back;

for i=2:96
    back_pat_tmp = circshift(pattern_back, i - 1, 2);
    patternTemp(:, :, i) = back_pat_tmp;
end

j = 1;
k = 1;
for i=1:96
    pattern.Pats(:, :, j, k) = patternTemp(:, :, i);
    k = k+1;
    if k > 10
        j = j + 1;
        k = 1;
