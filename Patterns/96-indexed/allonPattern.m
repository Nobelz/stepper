...
%         pattern.x_num,pattern.y_num);
    pattern.Pats = ones(2, 96, pattern.x_num,pattern.y_num);
%--------------------------------------------------------------------------
%SPECIFIC PARAMETERS
    pattern.name = 'nobelAllOn';
...
% % pattern.Pats(:,:,1,1) = fsdofilt(pattern.Pats(:,:,1,1),0.20,0.45);
% 
% % pattern_back=pattern.Pats(:,:,1,1);
% % %pattern_back =onef;
% % pattern_bar=pattern.Pats(:,:,1,1);
% % back_pat=pattern_bar;
% 
% %this is ones where the bar is and zeros elsewhere.
% bar_mask=zeros(size(pattern.Pats(:,:,1,1)));
% bar_mask(:,...
%     pattern.params.barstart:pattern.params.barend)=...
%     ones(size(bar_mask(:,...
%     pattern.params.barstart:pattern.params.barend)));        
% 
% bar_pat = bar_mask.*pattern_bar;
% back_mask   =   ones(size(pattern.Pats(:,:,1,1)));

% for i=1:96; %NOTE: msback and msbar are the same length.
%     %bar_pat_tmp = circshift(bar_pat,[0 i]);
%      bar_pat_tmp = circshift(bar_pat,[0 0]); %No bar! 6/2023
%     %back_mask_tmp = circshift(back_mask,[0 i]);
%      back_mask_tmp = circshift(back_mask,[0 0]);%No bar!
% 
%     for j=1:96;
%         back_pat_tmp = circshift(back_pat,[0 j]);
%         back_pat_tmp = back_pat_tmp.*back_mask_tmp;
%         pattern.Pats(:,:,i,j) = back_pat_tmp;
%     end
% end
...
% %make pattern 1D
% 
% for(i=1:size(pattern.Pats,1));
%     pattern.Pats(i,:)=pattern.Pats(1,:);
% end;
...
% 
% %Save Pattern w/in 'Patterns'
% pattern.filename=['C:\Users\FoxLab-Hotel\Desktop\Panels\M2-' pattern.name '_neg'];
% save(pattern.filename, 'pattern');
% 
