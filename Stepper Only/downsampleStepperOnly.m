function downsampleStepperOnly(folderName)
% downsampleStepperOnly.m
% Downsamples head angles to match m-sequence length for stepper only 
% trials (~51 Hz)
%
% Inputs:
...
for i = 1 : length(procFiles)
    ...
    % Save to file
    save(strcat(PATH, folderName, '/', markerFiles(i).name(1 : end - 4), '_DOWNHEAD'), 'downhead');
end
end